...
class Referrals(object):
    def __init__(self):
        self.user_df = pd.read_csv('../data/u.csv',
                                   sep=',',
                                   ...
                                   dtype={'UserID': 'object',
                                          ...
                                          'FacebookID': 'object',
                                          'CircleIDs': 'object'})
        self.weeks = pd.read_csv('../data/week_counts.csv',
                                 sep=',',
                                 error_bad_lines=False,
                                 header=None)
    ...
    def join_referrals_weeks(self):
        '''
        Returns a dataframe with user IDs and the number of referral weeks
        they have (the number of total weeks spent on the app by everyone
        they have referred/invited)
        '''
        referrals = self.user_df[['UserID', 'SourceUserID']].dropna()
        referral_weeks = referrals.merge(self.weeks, how='inner',
                                         left_on='UserID', right_on='user_id')
    ...
    def identify_priority_users(self, referrers_df):
        ...
        #Setting priority users to those who have referred at least 10 others
        referrers['Priority'] = referrers['Referrals'].apply(lambda x: x >= 10)

        #Trying various limits for priority number of referrals
        for i in range(10):
            referrers['Priority' + str(i + 1)] = referrers['Referrals'].apply(lambda x: x >= i + 1)
